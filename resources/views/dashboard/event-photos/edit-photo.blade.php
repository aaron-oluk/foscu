...
<div class="mx-auto max-w-2xl space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-semibold text-slate-900">Edit photo</h2>
            <p class="mt-1 text-sm text-slate-500">Update {{ $photo->title }}.</p>
        </div>
        <a href="{{ route('dashboard.event-photos.index') }}" class="admin-btn-secondary">Back</a>
    </div>

    <div class="admin-card p-6">
        <form action="{{ route('dashboard.event-photos.photos.update', $photo->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            @method('PUT')
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    <label for="title" class="admin-label">Photo title *</label>
                    <input type="text" id="title" name="title" value="{{ old('title', $photo->title) }}" required class="admin-input @error('title') border-red-500 @enderror">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3" class="admin-input @error('description') border-red-500 @enderror">{{ old('description', $photo->description) }}</textarea>
                @error('description')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="admin-label">Current image</label>
                <img src="{{ $photo->image_url }}" alt="{{ $photo->alt_text }}" class="mt-1 h-32 w-48 rounded border border-slate-200 object-cover">
                <label for="image" class="admin-label mt-3">Replace image (optional)</label>
                <input type="file" id="image" name="image" accept="image/*" class="admin-file @error('image') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="text" id="alt_text" name="alt_text" value="{{ old('alt_text', $photo->alt_text) }}" class="admin-input @error('alt_text') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="number" id="display_order" name="display_order" value="{{ old('display_order', $photo->display_order) }}" min="0" required class="admin-input @error('display_order') border-red-500 @enderror">
